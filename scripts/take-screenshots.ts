...
async function run() {
  ...
  try {
    ...
    console.error("Failed to load page. Is the server running?", err);
    process.exit(1);
  }
  ...
  await page.screenshot({
    path: path.join(screenshotsDir, "home-populated-light.png"),
  });
  ...
  if ((await readBtn.count()) > 0) {
    ...
    await page.screenshot({
      path: path.join(screenshotsDir, "article-view-light.png"),
    });
  } else {
  ...
  const themeBtn = page
    .locator("button")
    .filter({ has: page.locator(".lucide-sun") })
    .first();
  ...
  if ((await themeBtn.count()) > 0) {
    ...
    await page.screenshot({
      path: path.join(screenshotsDir, "article-view-dark.png"),
    });
    ...
    await page.screenshot({
      path: path.join(screenshotsDir, "home-populated-dark.png"),
    });
  } else {
