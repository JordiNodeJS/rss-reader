...
import { Feed, Article, getAllFeeds, addFeed, deleteFeed, addArticle, getArticlesByFeed, getAllArticles, updateArticleScrapedContent } from '@/lib/db';
...
export function useFeeds() {
  ...
  const refreshArticles = useCallback(async () => {
    if (selectedFeedId) {
      ...
      // Sort by date desc
      setArticles(loadedArticles.sort((a, b) => new Date(b.pubDate).getTime() - new Date(a.pubDate).getTime()));
    } else {
  ...
  const removeFeed = async (id: number) => {
    ...
    await refreshArticles();
      ...
      if (data.content) {
        ...
        setArticles(prev => prev.map(a => a.id === articleId ? { ...a, scrapedContent: data.content } : a));
